...
const tripsAPI = {
   ...
   // Check if trip is eligible for review
   checkReviewEligibility: async (tripId) => {
      try {
         const response = await fetch(`${API_BASE_URL}/trips/${tripId}/review-eligibility`);
         const data = await response.json();
         if (!response.ok) throw new Error(data.message || 'Failed to check eligibility');
         return data;
      } catch (error) {
